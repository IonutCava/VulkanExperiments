...
namespace Divide {
          ...
          bindings{ bindings } 
    {
        ...
        if (vkCreateDescriptorSetLayout(_device.device(),
    ...
    void DescriptorPool::freeDescriptors(std::vector<VkDescriptorSet>& descriptors) const {
        vkFreeDescriptorSets(_device.device(),
                             descriptorPool,
    ...
    bool DescriptorWriter::build(VkDescriptorSet& set) {
        if (pool.allocateDescriptorSet(setLayout.getDescriptorSetLayout(), set)) {
            overwrite(set);
            return true;
        }
    ...
    void DescriptorWriter::overwrite(VkDescriptorSet& set) {
        ...
        vkUpdateDescriptorSets(pool._device.device(), static_cast<uint32_t>(writes.size()), writes.data(), 0, nullptr);
    }
