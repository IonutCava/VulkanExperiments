...
namespace Divide {
    ...
    public:
        ...
        [[nodiscard]] VkDescriptorBufferInfo descriptorInfo(VkDeviceSize size = VK_WHOLE_SIZE, VkDeviceSize offset = 0);
        [[nodiscard]] VkResult invalidate(VkDeviceSize size = VK_WHOLE_SIZE, VkDeviceSize offset = 0);

        void writeToIndex(void* data, int index);
        VkResult flushIndex(int index);
        [[nodiscard]] VkDescriptorBufferInfo descriptorInfoForIndex(int index);
        [[nodiscard]] VkResult invalidateIndex(int index);
